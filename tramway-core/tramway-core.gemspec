$:.push File.expand_path("../lib", __FILE__)

# Maintain your gem's version:
require "tramway/core/version"

# Describe your gem and declare its dependencies:
Gem::Specification.new do |s|
  s.name        = "tramway-core"
  s.version     = Tramway::Core::VERSION
  s.authors     = ["Pavel Kalashnikov"]
  s.email       = ["kalashnikovisme@gmail.com"]
  s.homepage    = "https://github.com/kalashnikovisme/tramway-core"
  s.summary     = "Core for all Tramway Rails Engines"
  s.description = "Core for all Tramway Rails Engines"
  s.license     = "MIT"

  s.files = Dir["{app,config,db,lib}/**/*", "MIT-LICENSE", "Rakefile", "README.md"]

  s.add_dependency 'reform-rails', '0.1.7'
  s.add_runtime_dependency 'simple_form', '~> 3.5', '>= 3.5.0'
  s.add_runtime_dependency 'sass-rails', '~> 5.0', '>= 5.0.7'
  s.add_runtime_dependency 'bootstrap', '~> 4.1', '>= 4.1.0'
  s.add_runtime_dependency 'audited', '~> 4.5', '>= 4.5.0'
  s.add_runtime_dependency 'enumerize', '~> 2.1', '>= 2.1.2'
  s.add_runtime_dependency 'mini_magick', '~> 4.8', '>= 4.8.0'
  s.add_runtime_dependency 'rmagick', '~> 2.16', '>= 2.16.0'
  s.add_runtime_dependency 'loofah', '~> 2.2', '>= 2.2.1'
  s.add_runtime_dependency 'haml-rails', '~> 1.0', '>= 1.0.0'
end
